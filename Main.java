...
public class Main {
    ...
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        LinkedList<String> tooHeavy = new LinkedList<String>();
        for(HashMap.Entry <String, Integer> entry: shopCart.entrySet()){
            if(entry.getValue()>300)
                tooHeavy.add(entry.getKey());
        }
        deleter(shopCart,tooHeavy);
        return shopCart;
    }
    public static void deleter(HashMap<String, Integer> shopCart, LinkedList<String> tooHeavy){
        for(String elem: tooHeavy){
            shopCart.remove(elem);
        }
